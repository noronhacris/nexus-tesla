...
# ABAS DO SISTEMA
tab1, tab2, tab3, tab4 = st.tabs(["🎰 Loteria", "📜 Bíblia", "₿ Cripto", "🌍 Mercado Global"])
...
with tab4:
    ...
    ativo = st.selectbox("Ativo de Refúgio:", ["GC=F", "^GSPC", "EURUSD=X"]) # Ouro, S&P500, Euro
    data_g = yf.download(ativo, period="30d", interval="1d")
    st.area_chart(data_g['Close'])
    st.subheader("💰 Estratégia de Alto Retorno")
    st.info("Foque em ativos que apresentam suporte na Frequência 9. O mercado global de consumo segue ciclos de expansão e contração matemática.")
