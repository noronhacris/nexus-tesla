...
st.markdown("""
    ...
    .stButton>button:hover { 
        transform: translateY(-5px) scale(1.01); background: linear-gradient(135deg, #d4af37 0%, #f9e295 100%); color: #000 !important;
        box-shadow: 0px 15px 50px rgba(212, 175, 55, 0.4);
    }
    
    .card-quantum { border-radius: 30px; background: linear-gradient(145deg, #0f0f0f, #050505); padding: 40px; border: 1px solid #222; margin-bottom: 30px; box-shadow: 20px 20px 60px #000; }
...
def render_corretora_chart(ticker, nome):
    try:
        ...
        fig.update_layout(
            ...
            title=dict(text=f"TERMINAL ANALÍTICO: {nome.upper()}", font=dict(color='#d4af37', size=22, family='JetBrains Mono'))
        )
        ...
        st.warning("🔄 Reconectando ao terminal de dados...")
...
def logic_astrolabio_tesla(max_n, qtd, modalidade):
    with st.status("🌀 SINCRO-VÓRTICE ATIVO...", expanded=True) as status:
        ...
        # GARANTIA DE POPULAÇÃO: Nunca dá erro de "sample larger than population"
        if qtd > len(vortex_base):
            pool_extra = list(set(populacao_total) - set(vortex_base))
            full_pool = vortex_base + random.sample(pool_extra, qtd - len(vortex_base))
        else:
...
with st.sidebar:
    ...
    st.write("---")
    st.markdown("**Status:** Operacional | **Nível:** Admin")
    st.caption(f"Sync: {datetime.now().strftime('%H:%M:%S')}")

# =================================================================
# 5. EXECUÇÃO DOS MÓDULOS (CONTEÚDO INTEGRAL)
# =================================================================

if menu == "💎 IA Quântico Tesla":
    st.title("💎 IA Quântico Tesla & Astrolábio")
    st.markdown("<div class='card-quantum'>Análise baseada na Matemática de Vórtice 3-6-9 de Nikola Tesla.</div>", unsafe_allow_html=True)
    ...
    if st.button("EXECUTAR CONFLUÊNCIA DE VÓRTICE"):
        ...
        st.markdown(f"<div class='card-quantum' style='text-align: center; border: 2px solid #d4af37;'><h1 style='font-size: 3.5rem;'>{', '.join(map(str, nums))}</h1></div>", unsafe_allow_html=True)
    ...
    st.markdown("<div class='card-quantum'>Monitoramento do Mercado Pet High-End e E-commerce especializado.</div>", unsafe_allow_html=True)
    pet_ativo = st.selectbox("Selecione Ativo:", ["PETZ3.SA (Petz Brasil)", "CHWY (Chewy USA)", "PAWZ (ETF Pet Care)"])
    ...
    with col_t1:
        st.markdown("<div style='background: #0a0a0a; padding: 25px; border: 1px solid #d4af37; border-radius: 20px;'><h4>🧠 INSIGHT IA</h4><p>Fluxo institucional detectado em zonas de liquidez.</p></div>", unsafe_allow_html=True)
    with col_t2:
        st.plotly_chart(go.Figure(data=[go.Bar(x=['BTC', 'ETH', 'USD', 'Gold'], y=[65, 78, 12, 8], marker_color='#d4af37')], layout=dict(template='plotly_dark', height=250, paper_bgcolor='rgba(0,0,0,0)')), use_container_width=True)

elif menu == "👗 Fashion High-Ticket":
    st.title("👗 Radar Fashion Luxo")
    f_marca = st.selectbox("Selecione a Marca:", ["MC.PA (LVMH)", "RMS.PA (Hermès)", "KER.PA (Kering)", "ARZZ3.SA (Arezzo)", "SOMA3.SA (Soma)"])
    render_corretora_chart(f_marca.split(" (")[1].replace(")", ""), f_marca)
    
    st.subheader("🏛️ TOP 10 CONGLOMERADOS DE LUXO")
    col_fi1, col_fi2 = st.columns([3, 2])
    marcas = ['LVMH', 'Hermès', 'Dior', 'Chanel', 'Richemont', 'Kering', 'Estée Lauder', 'Rolex', 'Prada', 'Burberry']
    valores = [420, 210, 150, 110, 85, 70, 65, 50, 45, 30]
    with col_fi1:
        st.plotly_chart(go.Figure(data=[go.Bar(x=marcas, y=valores, marker_color='#d4af37')], layout=dict(template='plotly_dark', title="Market Cap (Billion USD)")), use_container_width=True)
    with col_fi2:
        st.plotly_chart(go.Figure(data=[go.Pie(labels=marcas, values=valores, hole=.4)], layout=dict(template='plotly_dark', title="% Market Share")), use_container_width=True)
    ...
    st.subheader("🏛️ TOP 10 RESERVAS DE OURO (TONELADAS)")
    ...
    st.title("🙏 Sabedoria & Legado")
    st.markdown("""
        <div class='state-message'>
            <h2 style='text-align: center; color: #d4af37;'>O FUNDAMENTO DA SOBERANIA REAL</h2>
            <p align='center'><i>"Minha é a prata, e meu é o ouro, diz o Senhor dos Exércitos." (Ageu 2:8)</i></p>
            <hr style='border: 0.5px solid #333;'>
            <p>A riqueza sem propósito é apenas acúmulo. A verdadeira soberania vem da dependência da Fonte Primária.</p>
            <div style='text-align: center;'><h1 style='font-size: 5rem;'>Ω</h1><p>SOLI DEO GLORIA</p></div>
        </div>
    """, unsafe_allow_html=True)

elif menu == "🤝 Conselho de Elite":
    st.title("🤝 Conselho de Elite")
    st.markdown("<div class='card-quantum'>Protocolos de Gestão e Expansão.</div>", unsafe_allow_html=True)
    c1, c2 = st.columns(2)
    with c1:
        st.checkbox("Devocional e Alinhamento realizados")
        st.checkbox("Análise de Risco de Vórtice concluída")
    with c2:
        st.metric("META E-COMMERCE PET", "R$ 50k", "+12%")
        st.metric("RESERVA DE SOBERANIA", "15% Patrimônio", "Lastro Ouro")
